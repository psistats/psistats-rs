...
impl MqttWrapper {
  ...
  pub fn init(hostname: &str, conf: &HashMap<String, toml::Value>) -> Result<Self, PsistatsError> {
    ...
    let tcr = topic_commands_reports.clone();
            ...
            match n {
              Event::Incoming(incoming) => {
                match incoming {
                  ...
                  Packet::ConnAck(msg) => {
                    client.subscribe(tcr, QoS::AtMostOnce).unwrap();
                  }
                  _ => ()
                }
              },
    ...
    return Ok(MqttWrapper {
      ...
      topic_commands_reports: String::from(topic_commands_reports),
      topic_commands: String::from(topic_commands)
    });
  }
